...
############################################################
########### only vertical gradient for velocity ... if not rotation
############################################################
gammax=0.
print(' we must erase the horizontal gradient',gammax)
...
for ix in range(0,ntheta):
   ...
   ray_rk2=ray_velocity_gradient.rk2_ray(xsrc,zsrc,xmin,xmax,zmin,zmax,\
                   czero,gammax,gammaz,\
                   theta,dtau,nt_max,\
                   ioption,idebug)
...
plt.savefig("ray_velocity_2D.pdf",dpi=300,format='pdf')  
plt.show()
