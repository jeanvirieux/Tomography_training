...
recursive subroutine ray_segment(idata_cur,model_cur,xbeg,zbeg,xend,zend,xleng_total,time_syn,mesh,frechet_single,ioption,irecur)
  ...
  integer(kind=4) :: index1,index2,index3,index4
  real(kind=4) :: xxbeg,zzbeg,xxend,zzend
  real(kind=4) :: func_beg,func_end
  ...
  if(ixbeg == ixend .and. izbeg == izend) then   ! same cell for both ends of the ray segment
     ...
     if(ioption == 1) then
        ! reduced variables in the local cell! values should be between 0 and 1
        xxbeg=(xbeg-(ixbeg-1)*mesh%dx+mesh%xmin)/mesh%dx
        zzbeg=(zbeg-(izbeg-1)*mesh%dz+mesh%zmin)/mesh%dz
        xxend=(xend-(ixend-1)*mesh%dx+mesh%xmin)/mesh%dx
        zzend=(zend-(izend-1)*mesh%dz+mesh%zmin)/mesh%dz
        ! dof index for the four corners
        index1=index              ! left bottom corner  (i,j)
        index2=index+1            ! right bottom corner (i+1,j)
        index3=index+mesh%nx      ! left up corner      (i,j+1)
        index4=index3+1           ! right up corner     (i+1,j+1)
        !================================== assuming bilinear interpolation of the slowness
        func_beg=(1.0-xxbeg)*(1.0-zzbeg)
        func_end=(1.0-xxend)*(1.0-zzend)
        ! trapezoidal rule for frechet estimation
        frechet_single(index1)=frechet_single(index1)+xleng_segment*0.5*(func_beg+func_end)
        func_beg=xxbeg*(1.0-zzbeg)
        func_end=xxend*(1.0-zzend)
        frechet_single(index2)=frechet_single(index2)+xleng_segment*0.5*(func_beg+func_end)
        func_beg=(1.0-xxbeg)*zzbeg
        func_end=(1.0-xxend)*zzend
        frechet_single(index3)=frechet_single(index3)+xleng_segment*0.5*(func_beg+func_end)
        func_beg=xxbeg*zzbeg
        func_end=xxend*zzend
        frechet_single(index4)=frechet_single(index4)+xleng_segment*0.5*(func_beg+func_end)
     endif
...
subroutine slowness(x,z,ixbeg,izbeg,model_cur,mesh,xlenteur)
  ...
  xlenteur=xvalue(1,1)*(1.0d00-xx)*(1.0d00-zz)  &
          +xvalue(2,1)*    xx *(1.0d00-zz)  &
          +xvalue(1,2)*(1.0d00-xx)*    zz  &
          +xvalue(2,2)*    xx*     zz
return
