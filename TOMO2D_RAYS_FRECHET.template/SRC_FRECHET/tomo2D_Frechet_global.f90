...
program tomo2D_simple
  ...
  real(kind=4) :: fcost,seuil_velocity,seuil_slowness         ! cost function value and threshold at one point
  integer(kind=4) :: ioption,idata,iter_max_local,iter_max_global,init,action,index,imodel

  integer(kind=4) :: iter_global
  real(kind=4) :: seuil_vel
  logical :: convergence
  ...
  !&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
  read(unitf,*) iter_max_global,iter_max_local,damp
  read(unitf,*) seuil_vel
  seuil_slowness=1./seuil_vel         ! threshold in slowness (all points have a perturbation less than this value)
  ...
  if(ioption == 1 ) then
     ...
     !-------------------------------------------------------------------------!
     ! This is the starting of the big loop                                    !
     !-------------------------------------------------------------------------!
...
1000 continue
     ...
     if(inform%itn >= iter_max_local) then
        write(*,*) ' ending with maximum iterations without convergence',iter_max_local,inform%itn
     ...
     if(action == 2) then    ! data_cur= data_cur + model%frechet * model_cur    u= u + A v
        do idata=1,data%nobs
           do index=1,mesh%ndof
...
2000 continue
     ...
     write(7,rec=iter_global) sngl(model_upd(:))
     close(7)


! make a test here for going to 3000 if model_upd is too small 
     convergence=.false.
     if(iter_global >= iter_max_global) then
       convergence=.true.
       write(*,*) ' maximum iterations reached ',iter_global,iter_max_global
     endif
     if(minval(abs(model_upd)) <= seuil_slowness) convergence=.true.
     if(convergence) goto 3000

     write(config_file,3001) iter_global
3001 format('fcal_',I4.4,'.dat')
     goto 2100


3000 continue                      ! end of the global loop

     !===================================================================
     !=============@@@@@@@@@@@@@@@@@@@@@@@ output the final updated model
     !===================================================================
